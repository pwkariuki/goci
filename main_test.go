...
func TestRunKill(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Channels for communication with goroutines
			errCh := make(chan error)
			ignSigCh := make(chan os.Signal, 1) // SIGQUIT
			expSigCh := make(chan os.Signal, 1) // SIGINT, SIGTERM
			...
			// Execute `run` function sending the error to error channel
			go func() {
				errCh <- run(tc.proj, io.Discard)
			}()
			// Sends desired signal to test executable
			go func() {
